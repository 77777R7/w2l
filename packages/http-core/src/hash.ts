/**
 * SHA-256 (FIPS 180-4) and UTF-8 encoding, inlined.
 *
 * Lives in its own module so `compliance.ts` and `access.ts` can both hash
 * without importing each other. `http-core` is zero-dependency by contract —
 * it does not import `node:crypto` — so the primitive is written out here
 * rather than borrowed, and pinned against published test vectors.
 */
